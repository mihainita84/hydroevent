...
def prepare_series(
    ...
    if pd.isna(position):
        mask &= df["position"].isna()
    else:
        pos = pd.to_numeric(df["position"], errors="coerce")
        mask &= np.isclose(pos, float(position), equal_nan=False)
...
def analyze_event(
    ...
    # Weighted centroid. Timestamps are shifted to the center of each logger interval.
    weights = rain_event["value"].to_numpy(float)
    seconds = rain_event["datetime"].astype("int64").to_numpy(float) / 1e9
    centroid_seconds = float(np.average(seconds, weights=weights))
    centroid = pd.to_datetime(centroid_seconds, unit="s", utc=True)
